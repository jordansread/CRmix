...
#v3.2 (15-Feb-2013) Code not working for Jordan.  Error with diffinv().  Change diffinv() to cumsum() equivalent, except now the code will not work if Dt!=1.  Alternatively, can skip the simulation aspect of this code altogether.  That'll be _v3.3 if this version does not work.
Metabolism_LM <- function(X, X2=NULL, Dt=1, AtmPress=0.942*760, SondeZ=0.7, WindHeight=2, ExpectedFreq=288, ReturnEstimates=TRUE, ReturnFitted=FALSE, ReturnModels=FALSE){
	...
	SatdConc <- function(celcius, mmHg){
		...
		DOuM=DOmgL*1000/(15.999*2) #convert to µM
		return(DOuM)
	}
	...
	for(d in 1:max(DepID)){
		...
		if(d==1){
			#15-Feb-2013: Changed diffinv to cumsum() equivalent for Jordan.  This code will no longer work if Dt!=1
			# sBioDOd <- diffinv(Metab_rawd[,"NEP_raw"], Dt)+BioDO[ID][1]
			sBioDOd <- c(0,cumsum(Metab_rawd[,"NEP_raw"]))+BioDO[ID][1]
		}else{
			# sBioDOd <- diffinv(Metab_rawd[,"NEP_raw"], Dt)+sBioDOd[length(sBioDOd)] #BioDO[ID][1]
			sBioDOd <- c(0,cumsum(Metab_rawd[,"NEP_raw"]))+sBioDOd[length(sBioDOd)]
		}
		ThisK <- K[ID]
